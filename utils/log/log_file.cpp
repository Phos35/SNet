...
void LogFile::write(const char *val)
{
    ...
    ++written_times_;
    // printf("Logger written times: %lu, written_size: %lu\n", written_times_, written_size_);
    ...
    // 跨天滚动日志文件 
    else if(written_times_ >= LOG_ROLL_CHECK_EVERY_N && 
       Timestamp::is_different_day(create_time_, Timestamp::now()) == true)
    {
        roll_file();
    }
}
